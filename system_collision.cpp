...
void SystemCollision::HandleBallNetCollision()
{
	...
	if (cspr_ball->x >= 375 - 2*BALL_X_OFFSET && cspr_ball->x <= 375 && cspr_ball->y < 39 + BALL_X_OFFSET) {
		cmot_ball->v_x *= -1;
		cspr_ball->x = 375 - 2*BALL_X_OFFSET;
	}

	else if (cspr_ball->x >= 375 && cspr_ball->x <= 375 + 2*BALL_X_OFFSET && cspr_ball->y < 39 + BALL_X_OFFSET) {
		cmot_ball->v_x *= -1;
		cspr_ball->x = 375 + 2*BALL_X_OFFSET;
	}
	else if (cspr_ball->x >= 375 - 2*BALL_X_OFFSET && cspr_ball->x <= 375 + 2*BALL_X_OFFSET && cspr_ball->y == 39 + BALL_X_OFFSET) {
		cmot_ball->v_x *= -1;
		cmot_ball->v_y *= -1;
	}
